...
(find %{buildroot}{%{_bindir},%{_libexecdir}} -type f | grep -vE "archimport|svn|cvs|email|git-daemon" | sed -e s@^%{buildroot}@@) > bin-man-doc-files
(find %{buildroot}%{perl_vendorlib} -type f | sed -e s@^%{buildroot}@@) >> perl-files
%if %{!?_without_docs:1}0
(find %{buildroot}%{_mandir} -type f | grep -vE "archimport|svn|git-cvs|email|git-daemon" | sed -e s@^%{buildroot}@@ -e 's/$/*/' ) >> bin-man-doc-files
%else
...
rm -rf %{buildroot}/usr/share/git-gui
rm -rf %{buildroot}/usr/share/gitk
...
%changelog
* Thu Dec 23 2010 Sergio Rubio <rubiojr@frameos.org> 1.7.1-2.2
- FrameOS changes
